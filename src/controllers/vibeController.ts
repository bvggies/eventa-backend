...
export const rateEvent = async (req: AuthRequest, res: Response) => {
  try {
    ...
    await pool.query(
      ...
      [req.userId, eventId, rating, comment || null]
    );
    ...
    const avgResult = await pool.query(
      `SELECT AVG(CASE 
        ...
        ELSE 0
       END) as avg_rating
       FROM vibe_ratings WHERE event_id = $1`,
      [eventId]
    );

    const avgRating = parseFloat(avgResult.rows[0].avg_rating) || 0;
    await pool.query('UPDATE events SET vibe_rating = $1 WHERE id = $2', [
      avgRating,
      eventId,
    ]);
...
export const getEventRatings = async (req: any, res: Response) => {
  try {
    const { eventId } = req.params;
    const result = await pool.query(
      `SELECT vr.*, u.name as user_name, u.avatar as user_avatar
