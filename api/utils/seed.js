    ...
    try {
        ...
        for (const user of users) {
            ...
            if (existing.rows.length > 0) {
                ...
                if (user.is_admin) {
                    await client.query('UPDATE users SET is_admin = $1, is_organizer = $2 WHERE email = $3', [user.is_admin, user.is_organizer, user.email]);
                }
           ...
           VALUES ($1, $2, $3, $4, $5, $6)
           RETURNING id`, [user.name, user.email, user.password, user.phone, user.is_organizer, user.is_admin || false]);
        ...
        for (let i = 0; i < buzzPosts.length; i++) {
            ...
            await client.query(`INSERT INTO buzz_posts (user_id, content, event_id, hashtags)
         VALUES ($1, $2, $3, $4)`, [
                post.user_id,
                post.content,
