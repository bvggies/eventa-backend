...
const seedAllTables = async () => {
    ...
    try {
        ...
        await (0, database_2.initializeDatabase)();
        console.log('✅ Database tables initialized');
        console.log('');
        // Seed admin accounts
        console.log('📋 Step 2: Seeding admin accounts...');
        await (0, seedAdmin_1.seedAdminAccounts)();
        console.log('');
        ...
        const adminUserId = usersResult.rows.find((row) => row.email === 'admin@eventa.com')?.id;
        const regularUserId = usersResult.rows.find((row) => row.email === 'john@example.com')?.id;
        // Get event IDs
        ...
        if (parseInt(rsvpCount.rows[0].count) === 0 && eventIds.length > 0 && regularUserId) {
            for (let i = 0; i < Math.min(3, eventIds.length); i++) {
                await client.query(`INSERT INTO rsvps (user_id, event_id, status)
           VALUES ($1, $2, $3)
           ON CONFLICT (user_id, event_id) DO NOTHING`, [regularUserId, eventIds[i], i === 0 ? 'going' : 'interested']);
            }
        ...
        if (parseInt(ticketCount.rows[0].count) === 0 && eventIds.length > 0 && regularUserId) {
            for (let i = 0; i < Math.min(2, eventIds.length); i++) {
                ...
                await client.query(`INSERT INTO tickets (user_id, event_id, ticket_type, quantity, price, total, payment_method, payment_status, qr_code)
        ...
        if (parseInt(contactCount.rows[0].count) === 0 && regularUserId) {
            await client.query(`INSERT INTO trusted_contacts (user_id, name, phone, email, relationship, is_primary, can_receive_sos)
        ...
        if (parseInt(safetyCount.rows[0].count) === 0 && eventIds.length > 0 && regularUserId) {
            ...
            await client.query(`INSERT INTO safety_checks (user_id, event_id, status, alert_type, latitude, longitude, address, event_name, is_emergency, is_high_alert, google_maps_url)
        ...
        if (parseInt(venueCount.rows[0].count) === 0) {
            ...
            for (const venue of venues) {
                await client.query(`INSERT INTO afterparty_venues (name, type, location, latitude, longitude, rating, google_place_id, is_admin_added, added_by)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`, [
                    venue.name,
                    venue.type,
                    venue.location,
        ...
        if (parseInt(vibeCount.rows[0].count) === 0 && eventIds.length > 0 && regularUserId) {
            await client.query(`INSERT INTO vibe_ratings (user_id, event_id, rating, comment)
         VALUES ($1, $2, $3, $4)`, [
                regularUserId,
                eventIds[0],
                'lit',
...
exports.seedAllTables = seedAllTables;
// Run if called directly
if (require.main === module) {
    (0, exports.seedAllTables)()
        .then(() => {
